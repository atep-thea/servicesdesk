	...
	<div class="col-md-12">
		<div class="">
			<div class="box-header bg-title-form">
				<h3 class="box-title">Formulir Ubah Organisasi</h3>
			</div>
			<div class="box-body border-box">
		       ...
		       <form role="form" method="POST" action="{{route('organisasi.update', $organisasi->id_organisasi)}}" enctype="multipart/form-data">
					{!! csrf_field() !!}
                    {{ method_field('PUT')}}
				<div class="row top-22">
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="text" class="form-control" name="nama_opd" value="{{ $organisasi->nama_opd }}" required>
						</div>
						<div class="form-group">
							...
							<select class="form-control js-example-basic-single" name="induk_organisasi">
							   ...
							   @foreach($parent_org as $dataparent)
				                @if ($organisasi->induk_organisasi == $dataparent->nama_opd )
									<option value='{{ $dataparent->nama_opd }}' selected>{{ $dataparent->nama_opd }}</option>
								@else
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" name="nama_pengelola" value="{{ $organisasi->nama_pengelola}}">
						</div>
	                    <div class="form-group">
	                        <label>Nomer HP Pengelola</label>
	                        <input type="text" maxlength="13" class="form-control" name="no_hp_pengelola" value="{{ $organisasi->no_hp_pengelola}}" placeholder="max 13 angka">
	                    </div>
						<div class="form-group">
							<label>Email <a class="text-red">*</a></label>
							<input type="email" class="form-control" name="email" value="{{ $organisasi->email}}" required>
						</div>
						<div class="form-group">
