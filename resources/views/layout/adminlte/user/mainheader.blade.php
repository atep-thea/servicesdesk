...
<header class="main-header">
    ...
    <nav class="navbar navbar-static-top" role="navigation" style="background-color: #009788; color: #fff; ">
        ...
        @if (Auth::user()->role_user != 'User')
            <div class="portal">
                <a href="{{ url('/home') }}">Admin User &nbsp; <i class="fa fa-desktop"
        ...
        <div class="navbar-custom-menu">
            <ul class="nav navbar-nav">
                <li class="dropdown user user-menu">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="true"> <?php
$fotoProfil = 'public/photos/profile/' . Auth::user()->id . '/' . Auth::user()->foto;
?>
                        @if (empty($user->foto))
                            <img class="user-image" src="{{ asset('public/images/laki.jpg') }}" alt="">
                        @else
                            <img class="user-image" src="{{ asset($fotoProfil) }}" alt="">
                        @endif
                        <span class="hidden-xs">{{ Auth::user()->nama_depan }}
                            {{ Auth::user()->nama_belakang }} ({{ Auth::user()->role_user }})</span>
                    </a>
